...
export const useEditarSucursal = (onSuccess: (sucursal: Sucursal) => void) => {
  ...
  const editarSucursal = async () => {
    ...
    try {
      ...
      onSuccess(data)
      showToast(`Sucursal "${form.value.nombre_sucursal}" actualizada exitosamente`, 'success')
      dialog.value = false
    } catch (error) {
  ...
  return { dialog, loading, errorMessage, form, sucursalSeleccionada, abrirModal, editarSucursal }
}