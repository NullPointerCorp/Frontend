...
import { useEditarEmpleado } from "@/composables/useEditarEmpleado";
...
const {
  ...
  abrirModal,
  editarEmpleado,
} = useEditarEmpleado((empleado) => {
  emit("empleadoEditado", empleado);
});
...
const handleKeydown = (e: KeyboardEvent) => {
  ...
  if (e.key === "Enter" && !(e.target instanceof HTMLInputElement)) editarEmpleado();
};
  ...
  <v-dialog v-model="dialog" max-width="980" persistent>
    <v-card class="modal-card" theme="light">
      ...
      <div class="modal-actions">
        ...
        <v-btn class="save-btn" :loading="loading" @click="editarEmpleado">
